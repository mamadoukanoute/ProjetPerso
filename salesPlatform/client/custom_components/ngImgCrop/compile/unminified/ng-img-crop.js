  ...
  CropArea.prototype._preventBoundaryCollision=function(size) {
    ...
    //allow to set a user-defined aspect ratio for rectangles
      ...
      if (heightWithRatio < canvasH && se.y < canvasH) {
        //if the new height is smaller than canvas we modificate the size
        newSize.h = heightWithRatio;
      }
      else {
        // otherwise we modificate the width
         newSize.w = newSize.h * this._aspectRatio;
      }
      // this bug but add other bugs
      // if (heightWithRatio < canvasH && se.y < canvasH) {
      //   //if the new height is smaller than canvas we modificate the size
      //   newSize.h = heightWithRatio;
      // }
      // else if(widthWithRatio < canvasH && se.x < canvasW){
      //   // otherwise we modificate the width
      //    newSize.w = newSize.h * this._aspectRatio;
      // }
    }
  ...
  return function(ctx){
    ...
    this.drawCropArea=function(image, center, size, fnDrawClipPath) {
      ...
      if (size.w > 0 && size.w > 0) {
        if (size.w > ctx.canvas.width) size.w = ctx.canvas.width;
        if (size.h > ctx.canvas.height) size.h = ctx.canvas.height;

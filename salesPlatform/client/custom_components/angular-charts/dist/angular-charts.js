   ...
   */
     ...
     */
       ...
       */
        ...
        svg.append('g').attr('class', 'x axis').attr('transform', 'translate(0,' + height + ')').attr('font-size', '12').call(xAxis);
        svg.append('g').attr('class', 'y axis').attr('font-size', '12').call(yAxis);
        /**
      * Add bars
      * @type {[type]}
      */
        ...
        bars.attr('width', x0.rangeBand() - 100);
       ...
       */
        bars.on('mouseover', function (d) {
          makeToolTip({
            index: d.x,
            value: d.tooltip ? d.tooltip : d.x + d.y,
            series: series[d.s]
          }, d3.event);
       ...
       */
        if (config.labels) {
          barGroups.selectAll('not-a-class').data(function (d) {
            ...
            return height - Math.abs(y(d.y) - y(0) - 10);
          }).text(function (d) {
            return d.y;
          });
        }
        ...
        angular.forEach(linedata, function (value, key) {
          ...
          points.append('circle').attr('cx', function (d) {
            ...
            return function (d) {
              makeToolTip({
                index: d.x,
                value: d.tooltip ? d.tooltip : d.x + '<br>' + series + ': <b>' + d.y + '</b>',
                series: series
              }, d3.event);
              config.mouseover(d, d3.event);
              scope.$apply();
...
(function () {
    // styles.min.css
    var cssText = "" +
".angular-charts-template .axis path,.angular-charts-template .axis line{fill:none;stroke:#333}.angular-charts-template .ac-title{font-weight:700;font-size:1.2em}.angular-charts-template .ac-chart{float:left;width:70%}.angular-charts-template .ac-line{fill:none;stroke-width:2px}.angular-charts-template table{float:right;max-width:25%;list-style:none;margin:0;padding:0}.angular-charts-template td[ng-bind]{display:inline-block}.angular-charts-template .ac-tooltip{display:block;position:absolute;border:1px solid #333;background-color:#161616;border-radius:5px;padding:5px;color:#fff}";
    // cssText end
